...
public class Controller {
    ...
    public void handleSave() throws Exception {
        try {
            ...
            //     lastline = line;
            // }
            Recipe recipe = new Recipe(line[0], line[1], line[2]);
            String title = line[0];
            ...
            String response = model.performRequest("POST", null, null, title, ingredients, instructions,creationTime,null);            // Close the CSV reader
            reader.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public String handleGetAll() {
        String response = model.performRequest("GET", null, null, null, null, null,null, null);
        return response;
    }

    public String handlePut(String title, String ingredients, String instructions) {
        String response = model.performRequest("PUT", null, null, title, ingredients, instructions,null, null);
        return response;
    }

    public String handleDelete(String title) {
        String response = model.performRequest("DELETE", null, null, title, null, null, null, null);
        return response;
    }
}